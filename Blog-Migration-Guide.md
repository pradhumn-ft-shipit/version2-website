...
> - **`public/blog-data/`, `public/news-data/`, `public/sitemap.xml` are
>   git-ignored build output** — wiped and regenerated by `npm run blog:build`
>   on every build, never committed. Each build clears `public/blog-data/posts/`,
>   renders all Markdown, then copies `content/legacy-posts/*.json` in.
> - **`public/blog-images/` stays tracked** (not regenerable from tracked sources).
